...
module.exports = class MdToNotion {
  ...
  #findPage = async (pageTitle) => {
    ...
    const pageInCurentDataBase = getOnlyPage.filter(
      (e) => e.parent.database_id.replace(/-/g, "") == this.#databaseId
    );
    const page = pageInCurentDataBase.filter(
      (e) => e.properties.Name.title[0].plain_text == pageTitle
    );
    if (page.length == 0) {
      ...
      console.log(
        `Found "${page[0].properties.Name.title[0].plain_text} in database"`
      );
      return {
        title: pageTitle,
        id: page[0].id,
  ...
  #parserToRichTextObj = async (text) => {
    ...
    for (let i = 0; i < listOfText.length; i++) {
      if (regex.bold.test(listOfText[i])) {
        ...
        regex.backLink.test(listOfText[i]) &&
        !isNotMention.test(listOfText[i])
      ) {
        ...
        if (this.#databaseId !== null && this.#backlinkType !== null) {
          ...
          if (this.#backlinkType == "mention") {
            ...
            if (pageId.id != null) {
              //turn matched page into mention notion style
              mentionObj.mention.page.id = pageId.id;
              modifiedText.push(mentionObj);
              console.log(`Mention to ${link} complete`);
            } else {
              //if page not found create new one in current database instead
          ...
          } else if (this.#backlinkType == "link") {
            //user input "link"
            const richTextObj = new NotionObject().richTextObj;
            richTextObj.plain_text = content;
            richTextObj.text.content = content;
  ...
  #parserMdToNotionObj = async (text) => {
    ...
    if (bulleted_list_item.test(text)) {
      ...
      blockObj[0].bulleted_list_item.text = await this.#parserToRichTextObj(
        content
      );
      return blockObj;
    }
    ...
    if (numbered_list_item.test(text)) {
      ...
      blockObj[0].numbered_list_item.text = await this.#parserToRichTextObj(
        content
      );
      return blockObj;
    }
    ...
    if (table.test(text)) {
      ...
      //remove |---|
      const removeHyphen = splitColumn.filter(
        (e) => e.match(/\|.+\|/) && !e.match(/(?<=\|)\s*-+\s*(?=\|)/g)
      );
      let tableContent = "";
  ...
  #sameLevelCompress = async (listOfText) => {
    ...
    if (listOfText.length > 1) {
      ...
      //if input is just one line of text do this instead
    ...
    if (compressObj.length == 1) {
      ...
      for (let i = 0; i < compressObj.length; ) {
        ...
        if (compressObj[i].level - previous < 0) {
          ...
          if (
            compressObj[i - 1].level !== compressObj[i - 2].level &&
            isNoChild.length == 0
          ) {
            compressObj[i - 2].notionObj[postion][type]["children"] =
              compressObj[i - 1].notionObj;
            compressObj.splice(i - 1, 1);
          } else {
            //if previous is the same level of its parent or the parent can't have child, put them together.
          ...
          if (isNoChild.length !== 0) {
            ...
            compressObj[i - 1].notionObj[postion][type]["children"] =
              compressObj[i].notionObj;
  ...
  #findLastChild = async (parentPageId) => {
    ...
    const lastChildId =
      getChildlist.results[getChildlist.results.length - 1].id;
    const lastChildType =
      getChildlist.results[getChildlist.results.length - 1].type;
    return {
      id: lastChildId,
      type: lastChildType,
    };
  ...
  #modifiedInlineCodeBlock = (listOfString) => {
    ...
    for (i; i < listOfString.length; i++) {
      ...
      if (indexOfCode.length == 2) {
        ...
        const codeSplit = listOfString.splice(
          fistIndex,
          lastIndex - fistIndex + 1
        );
    ...
    for (let j = 0; j < listOfString.length; j++) {
      for (let i in regex) {
        while (listOfString[j].match(regex[i]) !== null) {
          ...
          const newReplce = listOfString[j].replace(
            regex[i],
            "┆" + textInside + "┆"
          );
  ...
  #getText = (filePath) => {
    ...
    listOfString = listOfString.filter(
      (e) => e !== "" && !/^\s+(?!.[^\s]*)/g.test(e)
    ); //remove blank line
    return listOfString;
  };
  ...
  uploadToPage = async (filePath, pageId) => {
    ...
    if (furthestLevel > 2) {
      ...
      for (let i = 0; i < compressObj.length; i++) {
        ...
        if (Level == 0) {
          uploadResponse = await this.#notion.blocks.children.append(
            this.#childObject(pageId, compressObj[i]["notionObj"])
          );
          arrOfEachLevelId = [pageId];
          previousLevel = 0;
        } else if (Level - previousLevel == 1) {
          const lastChildIdToAppend = await this.#findLastChild(
            arrOfEachLevelId[Level - 1]
          );
          ...
          const isNoChild = noChild.filter(
            (e) => e == lastChildIdToAppend.type
          );

          if (isNoChild.length == 0) {
            uploadResponse = await this.#notion.blocks.children.append(
              this.#childObject(lastChildId, compressObj[i]["notionObj"])
            );
            arrOfEachLevelId.push(lastChildId);
            previousLevel = Level;
          } else {
            //if target block can not have chile, append to previous parent.
            uploadResponse = await this.#notion.blocks.children.append(
              this.#childObject(
                arrOfEachLevelId[previousLevel],
                compressObj[i]["notionObj"]
              )
            );
          }
      ...
      uploadResponse = await this.#notion.blocks.children.append(
        this.#childObject(pageId, nestedObj)
      );
    }
  ...
  uploadToDatabase = async (filePath) => {
    ...
    if (pageId == null) {
      //if no page have found, then create new one, and upload the content.
  ...
  uploadFolder = async (folderPath) => {
    if (this.#backlinkType == null) {
      ...
      for (let i in files) {
        if (
          fs.lstatSync(path.resolve(folderPath, files[i])).isFile() &&
          (files[i].includes(".txt") || files[i].includes(".md"))
        ) {
          const upload = await this.uploadToDatabase(
            folderPath + "/" + files[i],
            this.#databaseId
          );
        }
    ...
    if (stats.size / (1024 * 1024) < 1) {
      try {
        ...
        if (error.message.match(/(?<=Response\scode\s)\w\w\w/)[0] == "417") {
          ...
          error.message.match(/(?<=Response\scode\s)\w\w\w/)[0] == "429"
        ) {
